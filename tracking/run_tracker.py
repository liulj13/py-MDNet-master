...
import sys
import time
import argparse     # it is a module for python to parse cmd parameter and options
import json
...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    # Init model
    model = MDNet(opts['model_path'])  # opts['model_path'] = '../models/mdnet_vot-otb.pth' in options.py
    if opts['use_gpu']:
        model = model.cuda()
    model.set_learnable_params(opts['ft_layers'])  # opts['ft_layers'] = ['fc'] in options.py
    
    # Init criterion and optimizer 
    criterion = BinaryLoss()  # defined in the model.py!
